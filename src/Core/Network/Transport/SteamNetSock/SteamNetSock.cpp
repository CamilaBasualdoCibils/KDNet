        ...
        SteamNetConnectionStatusChangedCallback_t* info)
{
      ...
      if (listener->connectionRequestCallback)
      {
        auto acceptCallback = [&]()
        {
          ...
          listener->_GetServerConnection(info->m_hConn)
              ->_ChangeState(SocketConnectionState::Connecting);
  ...
  else
  {
    ...
    if (it != clientConnections.end())
    {
      ...
      connection->_ChangeState(SocketConnectionState::Connecting);
    }
    else
    {
      assert(false &&
  ...
  if (ReceivedOnListener)
  {
    ...
    if (it != listeners.end())
    {
      ...
      if (connIt != listener->serverConnections.end())
      {
        ...
        connection->_ChangeState(SocketConnectionState::Connected);
      }
      else
      {
...
AtlasNet::Network::SteamNetSockTransport::Connect(const SocketAddress& address)
{
  ...
  SteamNetworkingIPAddr steamAddr = address.ToSteamAddr();
    logger->info("Connecting to {}:{}", address.to_string(),
                 steamAddr.m_port);
