...
void AtlasNet::RPCSystem_N::_SendCallRequest(
...
void AtlasNet::RPCSystem_N::_HandleCall(
...
void AtlasNet::RPCSystem_N::_HandleResponse(
...
AtlasNet::RPCSystem_N::RPCSystem_N(const Config& config) : config_(config)
{
...
void AtlasNet::RPCSystem_N::Bind(std::string_view name,
                                 RPC_BindCallFunction_Raw func)
{
  const RPCID id = RPC_Internal::HashRPCName(name.data());
  Bind(id, std::move(func));
}
void AtlasNet::RPCSystem_N::Bind(RPCID id, RPC_BindCallFunction_Raw func)
{
  std::unique_lock lock(mutex);
  bindings[id] = std::move(func);
}
void AtlasNet::RPCSystem_N::Call(SocketAddress target,
...
void AtlasNet::RPCSystem_N::Call(SocketAddress target, RPCID methodId,
                                 std::span<const uint8_t> payload)
{
...
AtlasNet::RPCSystem_N::Call_R(AtlasNet::SocketAddress target,
                              std::string_view methodName,
                              std::span<const uint8_t> payload)
{
  const RPCID methodId = RPC_Internal::HashRPCName(methodName.data());
...
[[nodiscard]] std::future<AtlasNet::RPCResult>
AtlasNet::RPCSystem_N::Call_R(AtlasNet::SocketAddress target, RPCID methodId,
                              std::span<const uint8_t> payload)
{
  RPCCallID callId = nextCallID++;
  ...
  newCall.onComplete = [promise = std::move(promise)](const pendingCall& call,
