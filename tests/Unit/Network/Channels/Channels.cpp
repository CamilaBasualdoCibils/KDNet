...
class MockClusterTransport : public Cluster::IClusterTransport
{
public:
  MockClusterTransport(PortType port,
                       std::shared_ptr<Cluster::IClusterResolver> resolver)
      : IClusterTransport(port, std::move(resolver))
  {
  }
...
TEST(Channels, V1ReliableReSend_FailedSend)
{
  ...
  std::shared_ptr<Cluster::UDPClusterTransport> transport =
      std::make_shared<Cluster::UDPClusterTransport>(listenPort, resolver);
  std::shared_ptr<MockClusterTransport> mockTransport =
      std::make_shared<MockClusterTransport>(listenPort, resolver);
